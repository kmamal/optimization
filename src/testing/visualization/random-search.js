...
const { init, iter } = require('../../random-search')
const {
...
const reset = async () => {
	...
	state = await init(
		problem,
		{
			accept: makeSimulatedAnnealing(kirkpatrickSchedule),
			temperature: 2,
			getNeighbor: (dst, src) => {
				__sampleHypercube(problem.order, dst, src, state.steps)
			},
			steps: getHalfRanges(problem.domain),
		},
	)
	...
	if (history.length >= 2) {
		for (let i = 1; i < history.length; i++) {
			...
			if (isLast) {
				...
				ctx.beginPath()
				ctx.arc(toX(state.candidate[0]), toY(state.candidate[1]), 5, 0, Math.PI * 2)
				ctx.stroke()
			}
		}
	}
